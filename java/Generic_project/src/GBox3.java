...
public class GBox3 {
  ...
  // 와일드카드 + 상한 제한(upper bound): Phone 또는 그 하위 타입(IPhone, Galaxy 등)만 허용
  ...
  // 와일드카드 + 하한 제한(lower bound): IPhone 또는 그 상위 타입(Phone, Object)만 허용
  // set()은 가능, get()은 Object로만 꺼낼 수 있음
  public void ddd(GBox<? super IPhone> box) {
    box.set(new IPhone());  // IPhone 또는 하위 타입은 넣을 수 있음
  }
